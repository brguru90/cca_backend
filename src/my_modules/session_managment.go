...
func (access_token AccessToken) GetSession(pointer_to_destination interface{}) error {
	return database_connections.RedisPoolGetJSON(access_token.Token_id+"_session", pointer_to_destination)
}
...
func (access_token AccessToken) SetSession(pointer_to_source interface{}) error {
	return database_connections.RedisPoolSetJSON(access_token.Token_id+"_session", pointer_to_source, time.Duration(time.Now().UnixMilli())-time.Duration(access_token.Exp))
}
